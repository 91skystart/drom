<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:53:"../application/index/tpl/dmdormitorymanage\index.html";i:1549872468;s:55:"D:\wwwroot\dorm\application\index\tpl\index\header.html";i:1548402963;s:53:"D:\wwwroot\dorm\application\index\tpl\index\menu.html";i:1545785666;s:55:"D:\wwwroot\dorm\application\index\tpl\index\footer.html";i:1543992410;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
  ...
  <div class="layui-body">
    ...
    <div style="padding: 15px;">
        ...
        <div class='search-box'>
          <div class="ipt-box">
              <!-- <form  action="<?php echo url('dmdormitorymanage/index'); ?>" method='post'>
                  <input placeholder="请输入姓名" value="<?php echo $name; ?>" name="name" class='search-ipt' type="text">
        ...
        <div style='margin-top:20px;' class='clearfix'>
                <div style='margin-bottom:20px;' class='nav-tab-box p-l'>
                    <span class='tab-item on'><a  href="<?php echo url('dmdormitorymanage/index'); ?>">添加人员</a></span>
                    <span class='tab-item'><a   href="<?php echo url('dmdormitorymanage/edit'); ?>">编辑人员</a></span>
                </div>
                <div class='p-r'>
                    <form id="search_from" action="<?php echo url('dmdormitorymanage/index'); ?>" method='post'>
                        <input class="ipt ipt-xs" name="job_num" value="<?php echo $job_num; ?>" placeholder="工号">
    ...
    <form name='form2' id="form2" encrypt="'multipart/form-data'">
        <div class='class-import-box-layer c-layer-padding'>
            <p style='padding:25px 10px;'>
                <span>下载导入模板：</span>
                <a href="/static/宿舍管理员信息.xls">宿舍管理员信息.xls</a>
            </p>
...
<script>
    ...
    $(document).on('click','.add',function(){
        ...
        if(data){
            $.post("<?php echo url('dmdormitorymanage/insert'); ?>",data,function(res){
                if(res.status == 1){
    ...
    $('.inp').click(function(){
        layer.open({
            ...
            success:function(){
                $("#file_11").on("change",function(){
                    ...
                    $.ajax({
                        url: "<?php echo url('index/dmdormitorymanage/importdmbuild'); ?>",
                        type: 'post',
                        data: formData,
                        dataType: 'json',
                        processData:false,
        ...
        $('.import_btn').on('click', function(){
            $.post("<?php echo url('index/dmdormitorymanage/imfile'); ?>", {'up_path': $(".up").val()}, function(res){
                if(res.code == 0){
                    layer.msg(res.msg, {icon:2});
                    setTimeout('location.reload()',3000);
                }else{
